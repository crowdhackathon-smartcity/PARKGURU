...
function Pen(map) {
    ...
    this.getData = function () {
        ...
        if (this.polygon != null) {
            ...
            paths.getAt(0).forEach(function (value, index) {
                returnData.push(readObject(value, index));
            });
            return returnData;
        }
        else if (gPolygon!=null && gPolygon != undefined) 
        {
            var returnData = [];
            var vertices = gPolygon.getPath();
            
            // Iterate over the vertices.
            vertices.forEach(function (xy, index) {
                returnData.push(readObject(xy, index));
            });
    ...
    function readObject(value, index) {
        var dataObj = {};
        dataObj["lng"] = value.lng();
        dataObj["lat"] = value.lat();
        return dataObj;
    }
}
